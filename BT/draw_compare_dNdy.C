...
/*------------------------------------------
  主函数
------------------------------------------*/
void draw_compare_dNdy() {

    gStyle->SetOptStat(0);

    /* ---------- 打开 实验数据 ---------- */
    TFile* fExp = TFile::Open(
        "./experimentData/ExpData_netP_dNdy_overNw.root",
        "READ"
    );
    ...
    /* ---------- energy tags ---------- */
    const int Nenergy = 3;
    const char* energyTag[Nenergy] = {"17.3", "62.4", "200"};
    ...
    const char* energyRange[Nenergy] = {
    ...
    /* ---------- Canvas ---------- */
    TCanvas* c = new TCanvas("c", "Data vs Model", 2500, 600);
    ...
    for (int ic = 0; ic < Ncent; ++ic) {
        ...
        frame->SetTitle(Form("Energy %s", centRange[ic]));
        ...
        if (ic == 0) {
            ...
            TLegendEntry* e =
            leg->AddEntry((TObject*)0, "Scale: p = N #times 0.5 ", "");
            e->SetTextColor(kRed+1);
            leg->Draw();
        }
    }

    c->SaveAs("compare_NA49_vs_model_BT_ScaleHalf.root");
}
